...
fig.suptitle(
    "Hate Speech Dataset — EDA (Ritesh Chouhan)",
    fontsize=13,
    fontweight="bold",
)
...
axes[1].bar(
    [orig_labels[i] for i in orig_counts.index],
    orig_counts.values,
    color=["#ff4560", "#ff9800", "#4caf50"],
    edgecolor="black",
    width=0.5,
)
...
for lbl, color in [(0, "#00e396"), (1, "#ff4560")]:
    axes[2].hist(
        ...
        alpha=0.65,
        color=color,
        label=f'{"Not Toxic" if lbl == 0 else "Toxic"}',
    )
...
plt.title(
    "Top N-grams in Toxic / Hate Speech Tweets",
    fontweight="bold",
)
...
for name, model in models.items():
    ...
    print(
        classification_report(
            y_test, y_pred, target_names=["Not Toxic", "Toxic"]
        )
    )
...
bars = plt.bar(
    results.keys(), results.values(), color=colors, edgecolor="black", width=0.5
)
plt.title(
